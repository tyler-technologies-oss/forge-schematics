...
export function moduleExists(tree: Tree, targetDir: string): boolean {
	return tree.getDir('/' + targetDir).subfiles.some(
		(p) => {
			...
			const isGenerated = readText(tree, `/${targetDir}/${p}`)?.includes('// This code was generated by the angular-custom-elements-schematic.  Any changes will be overwritten next time it runs.');
